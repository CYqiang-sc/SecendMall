...
import com.mall_server.mallserver.entity.*;
...
public class AdminProductController {
     ...
     * @return
     */
    @GetMapping("/updateAudit/{id}/state/{isAudit}")
    public R updateHot(@PathVariable(value = "id")Integer id,@PathVariable(value = "isAudit")Integer isAudit){
        Audit aud =  aduitService.getOne(new QueryWrapper<Audit>().eq("productId", id).orderByDesc("id"));
        aud.setIsAudit(isAudit);
        aduitService.saveOrUpdate(aud);
        Product product = productService.getById(id);
        product.setIsAudit(isAudit);
     ...
     * @return
     */
    @RequestMapping("/audit{productId}")
    public R secAudit(@PathVariable(value = "productId")Integer productId){
        Map<String,Object> resultMap=new HashMap<>();

        Audit Result = aduitService.getOne(new QueryWrapper<Audit>().eq("productId", productId).orderByDesc("id"));

//        QueryWrapper<Audit> warp =  new QueryWrapper<Audit>().eq("userId", 1).orderByDesc("id");
//        Audit one = aduitService.getOne(warp);

        if(Result != null){
            resultMap.put("id",Result.getId());
            resultMap.put("productId",Result.getProductId());
            resultMap.put("userId",Result.getUserId());
            resultMap.put("isAudit",Result.getIsAudit());
            resultMap.put("productName",Result.getProductName());
            resultMap.put("auditMsg",Result.getAuditMsg());
        }else {
    ...
    public R auditsave(@RequestBody Audit audit){

            aduitService.saveOrUpdate(audit);
//        if(audit.getId()==null || audit.getId()==-1){
//            aduitService.add(audit);
//        }else{
//            aduitService.update(audit);
//        }
